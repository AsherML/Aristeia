...
function find_user($db, $user_id)
{
  $records = exec_sql_query(
    $db,
    "SELECT * FROM authors WHERE id = :user_id;",
...
?>
  ...
  <form class="login hidden" action="<?php echo htmlspecialchars($action) ?>" method="post" novalidate>
...
function create_account($db, $name, $username, $password, $password_confirmation)
{
  ...
  if (empty($username)) {
    ...
    $records = exec_sql_query(
      $db,
      "SELECT username FROM authors WHERE (username = :username);",
      array(
        ':username' => $username
      )
    )->fetchAll();
  ...
  if ($account_valid) {
    $result = exec_sql_query(
      $db,
      "INSERT INTO authors (name, username, password) VALUES (:name, :username, :password);",
      array(
        ':name' => $name,
        ':username' => $username,
...
//check if an author has already liked a book
function has_liked($db, $book_id){
  $liked_book_ids=exec_sql_query($db, "SELECT books.id FROM books INNER JOIN author_book_likes ON author_book_likes.book_id=books.id WHERE books.id=:book_id AND author_book_likes.author_id=:author_id", array(":book_id"=>$book_id, ":author_id"=>current_user()['id']));
  if(count($liked_book_ids)>0){
    return "disabled";
  }
  else{
    return "";
