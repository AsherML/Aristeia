...
if (preg_match('/^\/public\//', $request_uri)) {
  ...
  return False;
} else if (match_routes($request_uri, ['/', '/authors'])) {
  require 'pages/authors.php';
} else if(match_routes($request_uri, '/books')){
  require "pages/books.php";
} else if(match_routes($request_uri, '/author')){
  require "pages/author.php";
} else if(match_routes($request_uri, '/book')){
  require 'pages/book.php';
} else if(match_routes($request_uri, '/accountCreation')){
  require 'pages/accountCreation.php';
}else {
